...
def recherche_facture(cursor):
    ...
    user_input = (input("Votre numéro de facture : "),)
    ...
    fichier = input("Appuyez sur F pour avoir le resultat dans un fichier csv : ")
    if fichier =="F":
        with open ('data.csv', 'w', newline='') as file:
            writer = csv.writer(file)
            header= ['num_facture', 'date_emission', 'email', 'raison_social']
            writer.writerow(header)
            writer.writerow(resultat_facture)
            file.close()  
    else:
        print(f'{line_gra*graph_resultat} \n N°: {resultat_facture[0]} | Date : {resultat_facture[1]} | E-mail : {resultat_facture[4]} | TOTAL : {resultat_facture[3]} € \n{line_gra*graph_resultat} ' )  
...
def recherche_fature_date(cursor):
    ...
    resultat_facture = cursor.fetchall()

    fichier = input("Appuyez sur F pour avoir le resultat dans un fichier csv : ")
    if fichier =="F":
        with open ('data.csv', 'w', newline='') as file:
            for row in resultat_facture:
                writer = csv.writer(file)
                header= ['num_facture', 'date_emission', 'email', 'raison_social']
                writer.writerow(header)
                writer.writerows(resultat_facture)
            file.close()  
    else: 
        for row in resultat_facture:
...
def load_csv_clients(db, cursor):
    ...
    with open(db, "r") as f:
        ...
        next(csv_data)
        for row in csv_data:
            cursor.execute('INSERT INTO clients(client_id, \
                type_client, raison_sociale, email, adresse, tel  )' \
                'VALUES(%s, %s, %s, %s, %s, %s)', 
                row)
        connection.commit()

def load_csv_factures(db, cursor):
    """ This function load and read the csv file, and insert row in db file.
    ...
    with open(db, "r") as f:
        ...
        next(csv_data)
        for row in csv_data:
            cursor.execute('INSERT INTO factures(num_facture, date_emission,\
                 Fk_client_id )' \
                'VALUES(%s, %s, %s)', 
                row)
        connection.commit()

def load_csv_produits(db, cursor):
    """ This function load and read the csv file, and insert row in db file.
    ...
    with open(db, "r") as f:
        ...
        next(csv_data)
        for row in csv_data:
            cursor.execute('INSERT INTO produits(produit_id, nom_produit,\
                ref_produit, prix_unite )' \
                'VALUES(%s, %s, %s, %s)', 
                row)
        connection.commit()

def load_csv_liens(db, cursor):
    """ This function load and read the csv file, and insert row in db file.
    ...
    with open(db, "r") as f:
        ...
        next(csv_data)
        for row in csv_data:
            cursor.execute('INSERT INTO liens_produits_factures(Fk_facture_id, \
                Fk_produit_id, quantite )' \
                'VALUES(%s, %s, %s)', 
                row)
        connection.commit()
...
try:
    ...
    remplir= input(f"{j}Voulez-Vous charger des données dans la base? (uniquement si base vide){f} ({rose}Y/N{f})?")
    if remplir == 'Y':
        db_clients = 'clients_100.csv'
        load_csv_clients(db_clients, cursor)
        db_factures = 'facture_client.csv'
        load_csv_factures(db_factures, cursor)
        db_produits = 'produits.csv'
        load_csv_produits(db_produits, cursor)
        db_liens = 'liens_produit_facture.csv'
        load_csv_liens(db_liens, cursor)
    else:
        pass
    suite ='Y'
    while True:
